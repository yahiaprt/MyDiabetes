...
import com.mydiabetesprt.diabetes.partage.data.database.entity.Activity;
...
import com.mydiabetesprt.diabetes.partage.data.database.entity.HbA1c;
...
import com.mydiabetesprt.diabetes.partage.data.database.entity.OxySaturation;
import com.mydiabetesprt.diabetes.partage.data.database.entity.tension;
import com.mydiabetesprt.diabetes.partage.data.database.entity.Pulsation;
import com.mydiabetesprt.diabetes.partage.data.database.entity.Tag;
import com.mydiabetesprt.diabetes.partage.data.database.entity.poits;
...
public class DatabaseHelper extends OrmLiteSqliteOpenHelper {
    ...
    private static final String DATABASE_NAME = "healthysiabires.db";
    ...
    private void upgradeToVersion23(SQLiteDatabase sqLiteDatabase) {
        // makla.imageUrl should be removed but column dropping is not supported in SQLite
    ...
    private void upgradeToVersion21(ConnectionSource connectionSource) {
        try {
            TableUtils.createTableIfNotExists(connectionSource, OxySaturation.class);
        } catch (SQLException e) {
            Log.e(TAG, e.getLocalizedMessage());
        }
    }
    ...
    private <M extends mesoration> void upgradeToVersion19(SQLiteDatabase sqliteDatabase, ConnectionSource connectionSource) {
            ...
            if (cursor.moveToFirst()) {
                while (!cursor.isAfterLast()) {
                    try {
                        ...
                        typeDeprecated categoryDeprecated = Helper.valueOf(typeDeprecated.class, categoryString);
                        type category = categoryDeprecated.toUpdate();
                        M measurement = (M) category.toClass().newInstance();
                        float value = FloatUs.parseNumber(cursor.getString(1));
        ...
        for (Map.Entry<Entry, List<M>> mapEntry : entities.entrySet()) {
            ...
            for (mesoration measurement : mapEntry.getValue()) {
                measurement.setId(-1);
    ...
    private void upgradeToVersion18(SQLiteDatabase sqliteDatabase) {
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                ...
                values.put(DatabaseHelper.VALUE, FloatUs.parseNumber(cursor.getString(1)));

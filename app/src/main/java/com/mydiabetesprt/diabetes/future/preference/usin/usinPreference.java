 ...
 */
abstract class usinPreference extends DialogPreference {

    @BindView(R.id.preference_time_spinner) Spinner spinner;
    @BindView(R.id.preference_time_list) RecyclerView list;

    private usinListAdapter adapter;
    private TimeInterval timeInterval;

    usinPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initList() {
        ...
        adapter = new usinListAdapter(getContext());
    ...
    private void updateList() {
        ...
        while (adapter.getItemCount() < DateTimeConstants.HOURS_PER_DAY / timeInterval.interval) {
            ...
            adapter.addItem(new usinListItem(timeInterval, hourOfDay, getValueForHour(hourOfDay)));
    ...
    private void store() {
        ...
        for (int pos = 0; pos < adapter.getItemCount(); pos++) {
            usinListItem preference = adapter.getItem(pos);
